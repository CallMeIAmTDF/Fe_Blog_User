import {
  ApiResponse,
  CategoryResponse, FollowStatsResponse,
  Notification,
  PageResponse,
  PostResponse,
  PostSummaryAIResponse,
  PostSummaryResponse, ToggleFollowResponse, UserResponse, UserUpdateRequest
} from "@/types/api";
...
class ApiService {
  ...
  private loadAccessToken() {
    this.accessToken = JSON.parse(<string>window.localStorage.getItem('authState'))?.accessToken || "";
  }
  ...
  async getUserFollowStats(userId: string) {
    return this.fetchApi<ApiResponse<FollowStatsResponse>>(`/blog/follow/${userId}`);
  }

  async getUserBlogs(userId: string, page = 0, size = 10) {
    return this.fetchApi<ApiResponse<PageResponse<PostSummaryResponse>>>(`/blog/post/user/${userId}?page=${page}`);
  }

  async checkFollowStatus(userId: string, pid: string) {
    if(pid != ""){
      return this.fetchApi<ApiResponse<boolean>>(`/blog/follow/isFollow?pid=${pid}`);
    }
    return this.fetchApi<ApiResponse<boolean>>(`/blog/follow/isFollow?userId=${userId}`);
  }
  ...
  async getBlogSummary(postId: string) {
    return this.fetchApi<PostSummaryAIResponse>(`/blog/post/summary?pid=${postId}`);
  }

  async postBlog(title: string, content: string, cids: number[], hashtags: string[], cover: string | null) {
    return this.fetchApi<ApiResponse<PostSummaryAIResponse>>(`/blog/post`, {
      method: 'POST',
      body: JSON.stringify({ title: title, content: content, cids: cids, hashtags: hashtags, cover: cover }),
    });
  }
  ...
  async getTopics() {
    return this.fetchApi<{ code: number; message: string; data: CategoryResponse[] }>(`/blog/category`);
  }
  ...
  async updateUser(user: UserUpdateRequest){
    return this.fetchApi<ApiResponse<UserResponse>>(`/users/${user.id}`, {
      method: "PUT",
      body: JSON.stringify({ name: user.name, dob: user.dob, gender: user.gender, avatar: user.avatar ??  ""}),
    })
  }

  async getUserById(uid: string) {
    return this.fetchApi<ApiResponse<UserResponse>>(`/users/fetchUserById/${uid}`);
  }
